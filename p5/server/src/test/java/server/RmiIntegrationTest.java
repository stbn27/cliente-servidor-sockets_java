...
class RmiIntegrationTest{
 @TempDir Path temp;
 @Test void flujoCrudViaStubRmi()throws Exception{int registryPort=freePort();ServerConfig config=new ServerConfig("127.0.0.1",registryPort,0,"TableroPrueba",temp.resolve("db/tablero"),Duration.ofMinutes(5));try(var server=ServidorApplication.start(config,false)){TableroNoticiasRemote stub=(TableroNoticiasRemote)LocateRegistry.getRegistry("127.0.0.1",registryPort).lookup("TableroPrueba");assertTrue(stub.verificarEstado().baseDatosDisponible());Sesion session=stub.iniciarSesion("redactor1","redactor123".toCharArray());Sesion other=stub.iniciarSesion("redactor2","noticias123".toCharArray());Noticia created=stub.publicarNoticia(session.token(),new NuevaNoticia("RMI real","Contenido remoto",Categoria.TECNOLOGIA));assertEquals(created.id(),stub.obtenerNoticia(created.id()).id());assertThrows(AutorizacionException.class,()->stub.editarNoticia(other.token(),created.id(),new ActualizacionNoticia("Ajena","Contenido",Categoria.CIENCIA),created.version()));Noticia edited=stub.editarNoticia(session.token(),created.id(),new ActualizacionNoticia("RMI editada","Contenido",Categoria.CIENCIA),created.version());assertThrows(ConflictoEdicionException.class,()->stub.editarNoticia(session.token(),created.id(),new ActualizacionNoticia("Vieja","Contenido",Categoria.CIENCIA),created.version()));stub.eliminarNoticia(session.token(),edited.id());assertThrows(NoticiaNoEncontradaException.class,()->stub.obtenerNoticia(edited.id()));}}
